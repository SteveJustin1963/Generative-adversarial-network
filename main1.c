...
// The code below is randomly initializing the weights of the first layer of a neural network.explain each line
// int i, j;
...
// the code below is for loop which iterates for every node in hidden layer
...
// the weights are initialized randomly between -1 and 1
 ...
 // the code below is creating a for loop that is going to iterate through the output size. 
...
// the code below is the forward function for a neural network. The function takes in the neural network, 
...
// where delta is the error gradient for the neuron in the hidden layer.  
 ...
 ;
 // The code is iterating through the hidden layer weights 
...
//  The code  iterates through the hidden layer and the output layer. 
